  ...
  <view class="con">
    <movable-area class="area" :style="{ height: areaHeight }" @mouseenter="mouseenter" @mouseleave="mouseleave">
      <block v-for="(item, index) in imageList" :key="item.id">
        ...
        >
          <view class="area-con isFlexCenter"  :style="{ width: childWidth, height: childWidth, transform: 'scale(' + item.scale + ')' }">
            <image class="pre-image" :src="item.src.src" mode="aspectFill"></image>
			<view>{{item.src.name}}</view>
           <view class="del-con" @click="delImage(item, index)" @touchstart.stop="delImageMp(item, index)" @touchend.stop="nothing()" @mousedown.stop="nothing()" @mouseup.stop="nothing()">
              <view class="del-wrap">
                <image class="del-image" src="/static/homeIcon/dragClose.png" v-if="type==0"></image>
				
				 <image class="del-image" src="/static/homeIcon/dragAdd.png" v-if="type==1"></image>
              </view>
            </view>
          </view>
        </movable-view>
  ...
  computed: {
    areaHeight() {
	  if(this.imageList.length==0){
		  ...
		  if (this.imageList.length < this.number -2) {
